# Manual Order Correction & Global AI Training
...
3.  **Confirm Training**: A toast message **"AI Training: Correction saved!"** will appear.
...
-   **Fuzzy Matching**: It uses a high-tolerance "Fuzzy Brain" that rounds coordinates to 10%. This means even if the AI detection is slightly jittery on a different page, it will recognize the pattern.
-   **Global Memory**: **Yes, it works across different manga.** If you train a "Sidebar on the Left" pattern in one title, the AI will use that same logic for a similar sidebar in any other title you read.
